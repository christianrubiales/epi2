...
public class DutchNationalFlagEPI {
	...
	private static void unitTest(int pivot, List<Color> A) {
		...
		new DutchNationalFlagEPI().dutchFlagPartition(pivot, A);
	...
	public static void main(String[] args) {
		...
		unitTest(1, Arrays.asList(Color.RED, Color.WHITE, Color.WHITE, Color.WHITE));
		unitTest(1, Arrays.asList(Color.RED, Color.BLUE, Color.WHITE, Color.WHITE));
		unitTest(0, Arrays.asList(Color.RED, Color.WHITE, Color.WHITE, Color.WHITE));
		...
		for (int times = 0; times < 10; ++times) {
			...
			new DutchNationalFlagEPI().dutchFlagPartition(pivotIndex, A);
			System.out.println(A);
	...
	public static boolean check(Color pivot, List<Color> A, List<Color> Adup) {
		...
		if (i != n) {
			System.err
					.println("Failing test: pivot = " + pivot + " array = " + Adup + "." + "Your code updated to " + A);
			System.exit(-1);
		}
		return true;
	}
