...
public class ReverseBitsEPI {
	  ...
	  public static void main(String[] args) {
	    ...
	    if (args.length == 2) {
	      ...
	      System.out.println("x = " + x + ", reverseBits(x) = " + Long.toBinaryString(reverseBits(x)));
	      System.out.println(Long.toBinaryString(reverseBits(x, 63)));
	      assert reverseBits(x) == reverseBits(x, 63);
	    } else {
	      Random r = new Random();
	      for (int times = 0; times < 1000; ++times) {
	        long x = times;
	        System.out.println("x = " + Long.toBinaryString(x) + ", reverseBits(x) = " + Long.toBinaryString(reverseBits(x)));
	        assert reverseBits(x) == reverseBits(x, 63);
	      }
	    }
	  }
